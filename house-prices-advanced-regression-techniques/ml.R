...
rmse_vals <- matrix(0, nrow = nfold, ncol = 2)
r2_vals <- matrix(0, nrow = nfold, ncol = 2)

# 5fold cross_validation, no hyperparameter tuning
# for SVM (kernel = radial, cost = 1) and default paramteres are selected

# out of 75 features, 48 features are selected using Step function. 
# we used only selected 48 features for LM, SVM.
...
for (fold in test_indx){
  ...
  model2 <- svm(best_formula, data=traindf[-fold, ]) # support vector machine (SVM)
...
colnames(rmse_vals) <- c('LinearRegression', 'SVM')

r2_vals <- data.frame(r2_vals)
colnames(r2_vals) <- c('LinearRegression', 'SVM')
